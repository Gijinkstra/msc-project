...
%         end
        A1=sum(signal(vec1+1).*exp(1i*phi(j)*vec1));
        B1=sum(signal(vec1+2).*exp(1i*phi(j)*vec1));
        C1=sum(signal(vec1+3).*exp(1i*phi(j)*vec1));
...
%         end
        A2=sum(signal(vec1+1).*exp(1i*phi(i)*vec1));
        B2=sum(signal(vec1+2).*exp(1i*phi(i)*vec1));
        C2=sum(signal(vec1+3).*exp(1i*phi(i)*vec1));
...
%         end
        A3=sum(signal(vec2+1).*exp(-1i*phi(j)*(M-vec2+1)));
        B3=sum(signal(vec2+2).*exp(-1i*phi(j)*(M-vec2+1)));
        C3=sum(signal(vec2+3).*exp(-1i*phi(j)*(M-vec2+1)));
...
%         end
        A4=sum(signal(vec2+1).*exp(-1i*phi(i)*(M-vec2+1)));
        B4=sum(signal(vec2+2).*exp(-1i*phi(i)*(M-vec2+1)));
        C4=sum(signal(vec2+3).*exp(-1i*phi(i)*(M-vec2+1)));
...
for i=1:J
    ...
    sumA=sum(vec4.*signal(vec3+1).*exp(1i*phi(i)*vec3));
    sumB=sum(vec4.*signal(vec3+2).*exp(1i*phi(i)*vec3));
    sumC=sum(vec4.*signal(vec3+3).*exp(1i*phi(i)*vec3));
